// flashattn_streaming_16x32_warp_softmax_v12b.cu
// v12b: warp-full softmax 유지 + M=16 공정비교용(ROWS_PER_BLOCK=4로 분할)
// - Each batch uses 4 CTAs (row blocks): rows [0..3], [4..7], [8..11], [12..15]
// - Block: 5 warps = 160 threads (warp0 loader + warp1..4 compute rows)
// - N_TILE=32, seq_len=128
//
// Build:
// nvcc -O3 -std=c++17 -arch=sm_86 -lineinfo    -o flashattn_streaming_16x32_warp_softmax_v12b.exe    flashattn_streaming_16x32_warp_softmax_v12b.cu
//
// Profile:
// ncu --set full --launch-skip 10 --launch-count 1    ./flashattn_streaming_16x32_warp_softmax_v12b.exe
...
// global problem (match old experiments)
constexpr int M_TOTAL = 16;
constexpr int Kdim    = 16;
constexpr int Dv      = 16;

constexpr int N_TILE  = 32;
constexpr int SEQ_LEN = 128; // must be multiple of 32
constexpr int NUM_TILES = SEQ_LEN / N_TILE;

// v12b tiling over rows
constexpr int ROWS_PER_BLOCK = 4; // 4 compute warps per CTA
...
// K_T: [B, SEQ_LEN, Kdim] half
// V:   [B, SEQ_LEN, Dv]   half
// O:   [B, M_TOTAL, Dv]   float
__global__ void flashattn_warp_full_softmax_v12b(
    ...
    int row_blk  = (int)blockIdx.y; // which group of 4 rows
    ...
    const __half* KT_b = K_T + (size_t)batch_id * seq_len * Kdim;
    const __half* V_b  = V   + (size_t)batch_id * seq_len * Dv;
    float*       O_b   = O   + (size_t)batch_id * M_TOTAL * Dv + row_base * Dv;

    __shared__ alignas(16) __half shK[N_TILE][Kdim + PAD_K];
    ...
    for (int t = 0; t < num_tiles; ++t) {
        ...
        // warp0: load 32 tokens of K_T and V into shared
        if (warp == 0) {
            // lane-pair mapping: (n=0..15, seg=0/1) and we load n and n+16
            int idx = lane;        // 0..31
            int n   = idx >> 1;    // 0..15
            int s   = idx & 1;     // 0/1
            ...
            // K_T token0/token1
            cp_async_cg_16B(&shK[token0][off], &KT_b[(base + token0) * Kdim + off]);
            cp_async_cg_16B(&shK[token1][off], &KT_b[(base + token1) * Kdim + off]);

            // V token0/token1
            cp_async_cg_16B(&shV[token0][off], &V_b[(base + token0) * Dv + off]);
            cp_async_cg_16B(&shV[token1][off], &V_b[(base + token1) * Dv + off]);
        ...
        // warp1..warp4: compute one row each (0..3)
        if (warp >= 1 && warp <= ROWS_PER_BLOCK) {
            int r = warp - 1; // row inside block

            float qk = 0.0f;
            #pragma unroll
            for (int k = 0; k < Kdim; ++k) {
                qk += __half2float(Q_b[r * Kdim + k]) * __half2float(shK[lane][k]);
            }
            qk *= scale;
...
static void transpose_K_host(
    ...
    for (int b = 0; b < num_batches; ++b) {
        const __half* Kb = hK.data()   + (size_t)b * Kdim * seq_len;
        __half*      Tb  = hK_T.data() + (size_t)b * seq_len * Kdim;
        for (int k = 0; k < Kdim; ++k) {
            for (int j = 0; j < seq_len; ++j) {
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like (v12b: warp-full softmax, M=16 via 4-row blocks, N_TILE=32)\n");
    ...
    std::vector<__half> hK_T ((size_t)num_batches * seq_len * Kdim);   // [L, Kdim]
    std::vector<__half> hV   ((size_t)num_batches * seq_len * Dv);
    ...
    dim3 block( (1 + ROWS_PER_BLOCK) * WARP_SIZE, 1, 1 ); // 5 warps = 160 threads
    dim3 grid(num_batches, M_TOTAL / ROWS_PER_BLOCK, 1);

    // correctness
    flashattn_warp_full_softmax_v12b<<<grid, block>>>(dQ, dKT, dV, dO, num_batches, seq_len, scale);
    ...
    // FLOPs rough:
    // QK: 2*M_TOTAL*seq_len*Kdim
    // PV: 2*M_TOTAL*seq_len*Dv
