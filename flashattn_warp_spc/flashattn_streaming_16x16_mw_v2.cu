// 8.x Streaming Softmax FlashAttention-like Multi-Warp (warp-parallel v2)
// - block당 2 warp (64 threads)
// - warp 0: rows 0..7, warp 1: rows 8..15 담당
// - QK^T: warp 0가 WMMA로 계산 → shared s_scores에 저장 → 두 warp가 같이 사용
// - Streaming softmax + PV: 각 warp가 자기 row range를 담당하되,
//   **row 내 계산은 warp 전체가 병렬로 수행 (lane 0 scalar가 아니라 warp-parallel)**.
...
// === Multi-warp fused kernel: QK^T (WMMA) + streaming softmax + PV (warp-parallel) ===
// Q:  [B, M, Kdim]       half
// K:  [B, Kdim, L]       half
// V:  [B, L, Dv]         half
// O:  [B, M, Dv]         float
__global__ void flashattn_streaming_16x16_kernel_mw_v2(
    ...
    for (int t = 0; t < num_k_tiles; ++t) {
        ...
        // 2) V tile load: block 전체 thread로 분산 (coalesced)
        __syncthreads();
        for (int idx = tid; idx < N_TILE * Dv; idx += blockDim.x) {
            int j = idx / Dv;    // 0..N_TILE-1 (seq index in tile)
            int d = idx % Dv;    // 0..Dv-1
            int kv_index = col_start + j;
            __half hv = V_b[kv_index * Dv + d];  // [L, Dv]
            sV_tile[d][j] = hv;
        }
        __syncthreads();
        ...
        if (lane < WARP_SIZE) {
            for (int i = row_start; i < row_end; ++i) {
                ...
                // --- 3-1) row-wise max (warp-parallel over columns) ---
                float x = NEG_LARGE;
                if (lane < N_TILE) {
                ...
                // --- 3-2) 이번 타일 softmax 기여 (warp-parallel) ---
                // 각 lane이 column subset을 담당
...
#pragma unroll
                    for (int d = 0; d < Dv; ++d) {
                        float v = __half2float(sV_tile[d][col]);
                        u_t_partial[d] += e * v;
                    }
                }
...
#pragma unroll
                for (int d = 0; d < Dv; ++d) {
                    float x_d = warp_allreduce_sum(u_t_partial[d]);
                    u_t[d] = x_d;
                }

                // --- 3-3) streaming update (lane 0, row i만 갱신) ---
                if (lane == 0 && l_t > 0.0f) {
                    ...
                    if (m_old == NEG_LARGE) {
                        ...
                        for (int d = 0; d < Dv; ++d) {
                            ...
                            float y_new = y_old * alpha + u_t[d] * beta;
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like Multi-Warp (WMMA QK^T, streaming softmax + PV) [warp-parallel v2]\n");
    ...
    // 1회 실행 (정확도 체크)
    flashattn_streaming_16x16_kernel_mw_v2<<<grid, block>>>(
        dQ, dK, dV, dO,
        num_batches,
        seq_len,
        scale
