// flashattn_streaming_16x16_mw_v4_vtile_swizzle.cu
// 8.2 v4: V tile DV-major swizzle (coalesced global load)
// - block당 2 warp (64 threads)
// - warp 0: rows 0..7, warp 1: rows 8..15 담당 (M=16)
// - QK^T: warp 0가 WMMA로 계산 → shared s_scores에 저장 → 두 warp가 같이 사용
// - V tile load: DV-major thread mapping + int4 vector load로 coalesced load 유도
// - Streaming softmax + PV: 각 warp가 자기 row range 처리, row 내 계산은 warp-parallel
// - O write: warp 내 lane-strided store
//
// 빌드 (예시):
// nvcc -O3 -std=c++17 -arch=sm_86 \
//   -o flashattn_streaming_16x16_mw_v4_vtile_swizzle.exe \
//   flashattn_streaming_16x16_mw_v4_vtile_swizzle.cu
//
// 프로파일 (예시):
// ncu --set full --launch-skip 10 --launch-count 1 \
//   ./flashattn_streaming_16x16_mw_v4_vtile_swizzle.exe
...
// ---------------- kernel ----------------
// Q: [B, M, Kdim] half
// K: [B, Kdim, L] half  (row-major with leading dim = L)
// V: [B, L, Dv] half
// O: [B, M, Dv] float
__global__ void flashattn_streaming_16x16_kernel_mw_v4(
    ...
    // tile scores
    __shared__ float s_scores[M * N_TILE];   // [M, 16]

    // V tile: [N_TILE][Dv] (DV-major)
    // padding으로 bank conflict 조금 완화 (optional)
    __shared__ __half sV_tile[N_TILE][Dv + 0];
    ...
    for (int t = 0; t < num_k_tiles; ++t) {
        ...
        if (warp_id == 0) {
            ...
            const __half* K_tile_base = K_b + col_start; // K[0, col_start]
        ...
        // 2) V tile load (coalesced, bug-fixed)
        // warp0: kv 0..7, warp1: kv 8..15
        int kv_base = warp_id * 8;          // 0 or 8
        int kv      = kv_base + (lane >> 2); // 0..15
        int d_base  = (lane & 3) * 4;        // 0,4,8,12  (4 half = 8B)

        if (kv < N_TILE) {
            int kv_global = col_start + kv;
            const __half* src = V_b + kv_global * Dv + d_base;

            // load 4 half (8 bytes) safely
            int2 v8 = *reinterpret_cast<const int2*>(src);
            *reinterpret_cast<int2*>(&sV_tile[kv][d_base]) = v8;
        }

        __syncthreads();

        // 3) streaming softmax + PV (warp-parallel)
        for (int i = row_start; i < row_end; ++i) {
            ...
            float u_t_partial[Dv];
            #pragma unroll
            for (int d = 0; d < Dv; ++d) u_t_partial[d] = 0.0f;
            ...
            for (int col = lane; col < N_TILE; col += WARP_SIZE) {
                ...
                #pragma unroll
                for (int d = 0; d < Dv; ++d) {
                    u_t_partial[d] += e * __half2float(sV_tile[col][d]);
                }
            }
            ...
            #pragma unroll
            for (int d = 0; d < Dv; ++d) {
                u_t[d] = warp_allreduce_sum(u_t_partial[d]);
            }

            // 3-3) streaming update (lane0)
            if (lane == 0 && l_t > 0.0f) {
                ...
                if (m_old == NEG_LARGE) {
                    ...
                    sh_l_running[i] = l_t;
                    #pragma unroll
                    for (int d = 0; d < Dv; ++d) sh_y_running[i][d] = u_t[d];
                } else {
                    ...
                    for (int d = 0; d < Dv; ++d) {
                        ...
                        sh_y_running[i][d] = y_old * alpha + u_t[d] * beta;
                    }
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like Multi-Warp (v4: V tile swizzle coalesced)\n");

    constexpr int NUM_BATCH   = 1024;
    constexpr int NUM_K_TILES = 8;       // seq_len = 128
    ...
    flashattn_streaming_16x16_kernel_mw_v4<<<grid, block>>>(dQ,dK,dV,dO,num_batches,seq_len,scale);
...
/*
빌드:
nvcc -O3 -std=c++17 -arch=sm_86   -o flashattn_streaming_16x16_mw_v4_vtile_swizzle.exe   flashattn_streaming_16x16_mw_v4_vtile_swizzle.cu

프로파일:
ncu --set full --launch-skip 10 --launch-count 1   ./flashattn_streaming_16x16_mw_v4_vtile_swizzle.exe
*/
