// 8.2-ish: V tile layout 개선 (half2 기반) + warp-parallel softmax/PV 유지
// - sV_tile을 __half2로 저장 (4-byte bank friendly)
// - V global load도 half2로 vectorize
// - PV 누적도 float2로 처리
...
static_assert(Dv % 2 == 0, "Dv must be even for half2");
...
__inline__ __device__ float2 warp_allreduce_sum_float2(float2 v) {
    v.x = warp_allreduce_sum(v.x);
    v.y = warp_allreduce_sum(v.y);
    return v;
}
...
// V: [B, L, Dv] half
// O: [B, M, Dv] float
__global__ void flashattn_streaming_16x16_kernel_mw_v3(
    ...
    int rows_per_warp = (M + num_warps - 1) / num_warps; // 8 if 2 warps
    ...
    // (Dv/2) x (N_TILE+1) half2 : 4-byte bank-friendly
    __shared__ __half2 sV2[Dv/2][N_TILE + 1];
    ...
    for (int t = 0; t < num_k_tiles; ++t) {
        ...
        // 2) V tile load (half2 vectorized)
        // load index space: N_TILE * (Dv/2)
        for (int idx = tid; idx < N_TILE * (Dv/2); idx += blockDim.x) {
            int j  = idx / (Dv/2);      // 0..15
            int d2 = idx % (Dv/2);      // 0..7  (each is two D components)
            int kv_index = col_start + j;

            // global: V_b[kv_index * Dv + (2*d2, 2*d2+1)]
            const __half* p = V_b + kv_index * Dv + (2 * d2);
            __half2 hv2 = *reinterpret_cast<const __half2*>(p);
            sV2[d2][j] = hv2;
        }
        ...
        for (int i = row_start; i < row_end; ++i) {
            ...
            float2 u2_partial[Dv/2];
            #pragma unroll
            for (int d2 = 0; d2 < Dv/2; ++d2) u2_partial[d2] = make_float2(0.f, 0.f);
            ...
            for (int col = lane; col < N_TILE; col += WARP_SIZE) {
                ...
                #pragma unroll
                for (int d2 = 0; d2 < Dv/2; ++d2) {
                    __half2 hv2 = sV2[d2][col];
                    float2 fv2  = __half22float2(hv2);
                    u2_partial[d2].x += e * fv2.x;
                    u2_partial[d2].y += e * fv2.y;
                }
            }
            ...
            #pragma unroll
            for (int d2 = 0; d2 < Dv/2; ++d2) {
                u2[d2] = warp_allreduce_sum_float2(u2_partial[d2]);
            }

            // 3-3) streaming update (lane0 only)
            if (lane == 0 && l_t > 0.0f) {
                ...
                if (m_old == NEG_LARGE) {
                    ...
                    #pragma unroll
                    for (int d2 = 0; d2 < Dv/2; ++d2) {
                        sh_y_running[i][2*d2 + 0] = u2[d2].x;
                        sh_y_running[i][2*d2 + 1] = u2[d2].y;
                    }
                } else {
                    ...
                    for (int d2 = 0; d2 < Dv/2; ++d2) {
                        ...
                        y0 = y0 * alpha + u2[d2].x * beta;
                        y1 = y1 * alpha + u2[d2].y * beta;
                        sh_y_running[i][2*d2 + 0] = y0;
                        sh_y_running[i][2*d2 + 1] = y1;
                    }
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like Multi-Warp (v3: V tile half2 SMEM)\n");
    ...
    flashattn_streaming_16x16_kernel_mw_v3<<<grid, block>>>(dQ,dK,dV,dO,num_batches,seq_len,scale);
...
/*
빌드:
nvcc -O3 -std=c++17 -arch=sm_86   -o flashattn_streaming_16x16_mw_v3_vtile_half2.exe   flashattn_streaming_16x16_mw_v3_vtile_half2.cu

프로파일:
ncu --set full --launch-skip 10 --launch-count 1   ./flashattn_streaming_16x16_mw_v3_vtile_half2.exe
*/
