// flashattn_forward_wmma_v2.cu
// 5.5.2: 멀티 워프 (4 warps) + 32x32 Tensor Core 타일 버전
//
// 빌드 예시 (Ampere, Windows PowerShell):
//   nvcc -arch=sm_86 -O3 -lineinfo -std=c++17 flashattn_forward_wmma_v2.cu -o flashattn_wmma_v2.exe
...
/**
 * FlashAttention Forward (Tensor Core / WMMA, 32x32 타일, 4워프)
 *
 * Q, K, V: [BH, N, D]  (half)
 * O:       [BH, N, D]  (float)
 *
 ...
 * D:  head dimension    (32 배수 가정, 16 배수는 필수)
 *
 * scale: 1/sqrt(D) 정도
 */
__global__ void flashattn_forward_wmma_v2_kernel(
    ...
    // softmax weights: [BLOCK_M, TILE_N] half
    ...
    // PV 임시: [BLOCK_M, D] float
    float* PV_smem = reinterpret_cast<float*>(smem_ptr);
    smem_ptr += BLOCK_M * D * sizeof(float);
    (void)PV_smem; // 컴파일러 경고 방지 (아래에서 사용)
    ...
    for (int col_start = 0; col_start < N; col_start += TILE_N) {
        ...
        // (1) K tile 로드: K[bh, col_start:col_start+tile_cols, :] -> K_smem_T[d, j]
        ...
        //
        //   Q_smem:   [BLOCK_M, D]   (32 x D)
        //   K_smem_T: [D, TILE_N]    (D x 32)
        //   -> S_smem: [BLOCK_M, TILE_N] float (32 x 32)
        //
        //   warp 매핑 (2x2 fragment grid, fragment size = 16x16):
        ...
        if (warp_id < NUM_WARPS) {
            ...
            for (int kk = 0; kk < D; kk += 16) {
                const half* a_ptr = &Q_smem[row0 * D + kk];
                const half* b_ptr = &K_smem_T[kk * TILE_N + col0];
            ...
            float* S_tile = &S_smem[row0 * TILE_N + col0];
            wmma::store_matrix_sync(S_tile, C_frag, TILE_N, wmma::mem_row_major);
        }
        __syncthreads();
        ...
        //
        //   W_smem: [BLOCK_M, TILE_N] (32 x 32)
        //   V_smem: [TILE_N, D]       (32 x D)
        //   -> PV_smem: [BLOCK_M, D]  (32 x D)
        //
        //   여기서는 D를 32 단위 타일로 나누고,
        //   warp 매핑은 (row, col) = (2 x 2) fragment grid (16x16)
        // ------------------------------------------------------
        // PV_smem 초기화
        for (int idx = tid; idx < BLOCK_M * D; idx += blockDim.x) {
            PV_smem[idx] = 0.0f;
        ...
        // D 방향을 32씩 타일링 (warp_n이 0: 0..15, 1: 16..31)
        const int OUT_TILE_D = 32; // 한 번에 처리하는 출력 D 범위
        for (int d_block = 0; d_block < D; d_block += OUT_TILE_D) {
            int d_rem = D - d_block;
            int cur_d = (d_rem >= OUT_TILE_D) ? OUT_TILE_D : d_rem;

            // cur_d < 32 인 tail의 경우, 일부 fragment가 OOB 될 수 있음
            // (간단하게 D가 32 배수인 경우를 우선 타겟으로 가정)
            if (cur_d < 16) break; // 너무 잘게 쪼개지면 스킵 (실용 범위 밖)
            ...
            if (warp_id < NUM_WARPS) {
                ...
                if (col0 >= D) {
                    ...
                    for (int kk = 0; kk < TILE_N; kk += 16) {
                        const half* w_ptr = &W_smem[row0 * TILE_N + kk];   // [BLOCK_M x TILE_N]
                        const half* v_ptr = &V_smem[kk * D + col0];        // [TILE_N x D]
                    ...
                    float* pv_ptr = &PV_smem[row0 * D + col0];
...
int main() {
    ...
    dim3 grid(ceil_div(N, BLOCK_M), BH, 1);   // x: row tiles, y: BH
    dim3 block(BLOCK_THREADS, 1, 1);          // 128 threads/block (4 warps)
    ...
    printf("Launching kernel v2 with grid=(%d,%d), block=%d, smem=%zu bytes\n",
           grid.x, grid.y, block.x, smem_bytes);

    int iters = 50;
    for (int i = 0; i < iters; ++i) {
        flashattn_forward_wmma_v2_kernel<<<grid, block, smem_bytes>>>(
...
# 간단 실행
.\flashattn_wmma_v2.exe

# ncu 프로파일 (31번째 런만)
ncu --set full --launch-skip 30 --launch-count 1 .\flashattn_wmma_v2.exe
