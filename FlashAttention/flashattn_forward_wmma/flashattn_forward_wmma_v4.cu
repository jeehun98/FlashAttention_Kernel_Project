// flashattn_forward_wmma_v4.cu
// 5.5.4: 멀티 워프 (4 warps) + 32x32 WMMA 타일 + warp-stripe shared access (bank conflict 완화)
//
// 빌드 예시 (Ampere, Windows PowerShell):
//   nvcc -arch=sm_86 -O3 -lineinfo -std=c++17 flashattn_forward_wmma_v4.cu -o flashattn_wmma_v4.exe
...
__global__ void flashattn_forward_wmma_v4_kernel(
    ...
    for (int col_start = 0; col_start < N; col_start += TILE_N) {
        ...
        //    각 warp는 r = warp_id, warp_id+NUM_WARPS, ... 행들을 처리
        // ------------------------------------------------------
        for (int r = warp_id; r < BLOCK_M; r += NUM_WARPS) {
            int global_row = row_start + r;
...
int main() {
    ...
    printf("Launching kernel v4 with grid=(%d,%d), block=%d, smem=%zu bytes\n",
...
# 실행
.\flashattn_wmma_v4.exe

# Nsight Compute (31번째 런만 프로파일)
ncu --set full --launch-skip 30 --launch-count 1 .\flashattn_wmma_v4.exe
*/
