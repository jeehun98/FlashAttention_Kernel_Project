// flashattn_streaming_16x16_mw_v6_3.cu
// v6.3: reduce shared bank conflicts
// - pad shV: [N_TILE][Dv+1]
// - pad s_scores: [M][N_TILE+1] and store_matrix_sync with ld = N_TILE+1
// - keep v6.2's 1x __syncthreads() per tile boundary (no mbarrier)
//
// Build:
// nvcc -O3 -std=c++17 -arch=sm_86 -lineinfo    -o flashattn_streaming_16x16_mw_v6_3.exe    flashattn_streaming_16x16_mw_v6_3.cu
//
// Profile:
// ncu --section SpeedOfLight --section MemoryWorkloadAnalysis --section WarpStateStats    --target-processes all --launch-skip 10 --launch-count 1    -o ncu_v6_3 ./flashattn_streaming_16x16_mw_v6_3.exe
// ncu --section MemoryWorkloadAnalysis --target-processes all --launch-skip 10 --launch-count 1   -o ncu_v6_3_fixed ./flashattn_streaming_16x16_mw_v6_3.exe
// ncu --import ncu_v6_3.ncu-rep --section WarpStateStats --page details
#include <cstdio>
#include <vector>
#include <random>
#include <cmath>
...
// ---------------- kernel ----------------
__global__ void flashattn_streaming_16x16_kernel_mw_v6_3(
    ...
    // AFTER: DvPad는 8의 배수로
    constexpr int DvPad = Dv + 8; // 24 (stride 48B, 16B aligned)
    __shared__ __half shV[2][N_TILE][DvPad];
    __shared__ float  s_scores[M][N_TILE + 1];         // v6.3: pad N_TILE

    int num_tiles = seq_len / N_TILE;

    // ---- helper: warp0 loads one tile t into buf ----
    auto load_tile_cpasync = [&](int t, int buf) {
        ...
        // NOTE: shV has stride (Dv+1), but we only store first Dv.
        int n = k;
        const __half* gV = V_b + (col_start + n) * Dv + off_h;
        __half* sV = &shV[buf][n][off_h];
        cp_async_cg_16B(sV, gV);

        // v6.3: commit once per warp (lane0 is enough)
        if (lane == 0) cp_async_commit_group();
    };
    ...
    for (int t = 0; t < num_tiles; ++t) {
        ...
        if (warp == 1) {
            ...
            // v6.3: store with padded ld = N_TILE+1
            wmma::store_matrix_sync(&s_scores[0][0], c_frag, N_TILE + 1, wmma::mem_row_major);

            // streaming softmax + PV
            for (int i = 0; i < M; ++i) {
                float x = (lane < N_TILE) ? (s_scores[i][lane] * scale) : NEG_LARGE;
                ...
                if (lane < N_TILE) {
                    float s = s_scores[i][lane] * scale;
                    float e = __expf(s - maxv);
                    l_part = e;
                    #pragma unroll
                    for (int d = 0; d < Dv; ++d) {
                        // v6.3: shV padded, access same but bank pattern changes
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like Multi-Warp (v6.3: pad shared to reduce bank conflicts)\n");
    ...
    // correctness
    flashattn_streaming_16x16_kernel_mw_v6_3<<<grid, block>>>(dQ,dK,dV,dO,num_batches,seq_len,scale);
    ...
    // perf (don't trust under heavy profiler)
