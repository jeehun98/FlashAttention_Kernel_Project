// flashattn_streaming_16x16_mw_v7_3_handshake_dbg.cu
// v7.3: ready/consumed handshake to prevent producer warp0 overwriting ping-pong buffers
// - block: 2 warps (64 threads)
// - warp0: loader (cp.async global->shared + wait_group0 + publish ready)
// - warp1: compute (WMMA QK^T + streaming softmax + PV + publish consumed)
// - no mbarrier; instead use volatile flags + threadfence_block + nanosleep spin
//
// Build:
// nvcc -O3 -std=c++17 -arch=sm_86 -lineinfo    -o flashattn_streaming_16x16_mw_v7_3_handshake_dbg.exe    flashattn_streaming_16x16_mw_v7_3_handshake_dbg.cu
//
// Profile (example):
// ncu --section SpeedOfLight --section MemoryWorkloadAnalysis --section WarpStateStats    --target-processes all --launch-skip 10 --launch-count 1    -o ncu_v7_3 ./flashattn_streaming_16x16_mw_v7_3_handshake_dbg.exe
...
#ifndef DEBUG_HANGHUNT
#define DEBUG_HANGHUNT 1
#endif

#ifndef WAIT_TIMEOUT_ITERS
#define WAIT_TIMEOUT_ITERS (1u << 20)   // ~1M spins
#endif
...
// V: [B, L, Dv]   half
// O: [B, M, Dv]   float
__global__ void flashattn_streaming_16x16_kernel_mw_v7_3(
    ...
    // ping-pong buffers (force 16B alignment to avoid cp.async misaligned issues)
    __shared__ __align__(16) __half shK[2][Kdim][N_TILE]; // [Kdim][N_TILE]
    __shared__ __align__(16) __half shV[2][N_TILE][Dv];   // [N_TILE][Dv]
    __shared__ float  s_scores[M * N_TILE];

    // Handshake flags (volatile)
    __shared__ volatile int sh_ready[2];     // produced tile index in each buffer
    __shared__ volatile int sh_consumed[2];  // last consumed tile index in each buffer

    if (tid == 0) {
        sh_ready[0] = -1; sh_ready[1] = -1;
        sh_consumed[0] = -1; sh_consumed[1] = -1;
    }
    __syncthreads();
    ...
    if (warp == 0) {
        ...
        if (batch_id == 0 && lane == 0) printf("[warp0][b0] start preload\n");
#endif
        load_tile_cpasync(0, 0);
        cp_async_wait_group0(); // issuer warp0 must wait
        if (lane == 0) {
            __threadfence_block();
            sh_ready[0] = 0;
#if ENABLE_DEBUG_PRINT
            if (batch_id == 0) printf("[warp0][b0] preload done, publish ready[0]=0\n");
#endif
        }
    }
    ...
    for (int t = 0; t < num_tiles; ++t) {
        ...
        // ---- warp1: wait until cur tile is ready, then compute it ----
        if (warp == 1) {
            unsigned it = 0;
            while (sh_ready[cur] != t) {
                ...
                if (++it > WAIT_TIMEOUT_ITERS) {
                    if (lane == 0) {
                        printf("[STUCK] batch=%d t=%d cur=%d ready0=%d ready1=%d cons0=%d cons1=%d\n",
                               batch_id, t, cur,
                               (int)sh_ready[0], (int)sh_ready[1],
            ...
            // publish consumed(cur)=t so warp0 can safely overwrite that buffer later
            if (lane == 0) {
                __threadfence_block();
                sh_consumed[cur] = t;
            }
        ...
        // ---- warp0: prefetch next tile into nxt, but DON'T overwrite until nxt was consumed ----
        if (warp == 0 && (t + 1) < num_tiles) {
            // nxt currently holds tile (t-1) at this point (for t>=1). Must be consumed before overwrite.
            if (t >= 1) {
                unsigned it = 0;
                while (sh_consumed[nxt] != (t - 1)) {
                    nanosleep_u32(64);
#if DEBUG_HANGHUNT
                    if (++it > WAIT_TIMEOUT_ITERS) {
                        if (lane == 0) {
                            printf("[PROD STUCK] batch=%d t=%d waiting consumed[%d]==%d (now %d) ready0=%d ready1=%d\n",
                                   batch_id, t, nxt, t-1,
                                   (int)sh_consumed[nxt],
                                   (int)sh_ready[0], (int)sh_ready[1]);
                        }
            ...
            // Wait for the just-issued group, then publish ready(nxt)=t+1
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like Multi-Warp (v7.3: ready/consumed handshake + dbg)\n");
    ...
    // correctness
    flashattn_streaming_16x16_kernel_mw_v7_3<<<grid, block>>>(dQ,dK,dV,dO,num_batches,seq_len,scale);
