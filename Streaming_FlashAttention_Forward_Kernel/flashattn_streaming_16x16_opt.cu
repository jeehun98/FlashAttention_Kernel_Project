// 6.xx Streaming Softmax FlashAttention-like (QK^T via WMMA, streaming softmax + PV, 개선 버전)
// - M = Kdim = Dv = 16 고정
...
//
// 개선 포인트:
// - running 상태(m,l,y)를 per-thread 배열이 아니라 shared memory로 이동 → 레지스터/스필 감소
// - per-tile local(m_t, l_t, y_t)는 row 단위 scalar/소형 배열로만 사용
// - V 타일 shared layout을 [Dv][N_TILE+1]로 패딩해서 bank conflict 완화
//
// 빌드:
// nvcc -O3 -std=c++17 -arch=sm_86 \
//   -o flashattn_streaming_16x16_opt.exe \
//   flashattn_streaming_16x16_opt.cu
//
// 프로파일:
// ncu --set full --launch-skip 10 --launch-count 1 ./flashattn_streaming_16x16_opt.exe
...
// V:  [B, L, Dv]         half
// O:  [B, M, Dv]         float
__global__ void flashattn_streaming_16x16_kernel_opt(
    ...
    // --- running 상태를 shared memory로 이동 ---
    __shared__ float sh_m_running[M];      // max
    __shared__ float sh_l_running[M];      // l = Σ exp(x - m)
    __shared__ float sh_y_running[M][Dv];  // y[d] = Σ exp(x - m) * v_d
    ...
    // shared: tile scores, V tile (bank conflict 완화를 위해 padding)
    __shared__ float  s_scores[M * N_TILE];          // [M, N_TILE]
    __shared__ __half sV_tile[Dv][N_TILE + 1];       // [Dv][N_TILE+1] (pad)
    ...
    for (int t = 0; t < num_k_tiles; ++t) {
        ...
        // 2) V_tile load: [N_TILE, Dv] → shared [Dv][N_TILE+1]
        {
            for (int idx = threadIdx.x; idx < N_TILE * Dv; idx += blockDim.x) {
                ...
                sV_tile[d][j] = hv;  // padded second dimension
            }
        }
        __syncthreads();

        // 3) tile 내 row별 local 통계(m_t, l_t, y_t) 계산 + streaming 업데이트
        //    - per-row scratch는 lane==0에서만 소형 배열로 유지
        if (lane < WARP_SIZE) {
            // row-wise 처리
            for (int i = 0; i < M; ++i) {
                // --- 3-1) row-wise max ---
                ...
                // --- 3-3) row-wise y_t[d] 계산 ---
                float y_t[Dv];       // lane==0만 의미 있게 씀
                if (lane == 0) {
                ...
                for (int d = 0; d < Dv; ++d) {
                    float contrib = 0.0f;
                    if (lane < N_TILE) {
                        __half hv = sV_tile[d][lane]; // [Dv][N_TILE+1], lane=j
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like OPT (WMMA QK^T, streaming softmax + PV)\n");
    ...
    // 1회 실행 (정확도 체크)
    flashattn_streaming_16x16_kernel_opt<<<grid, block>>>(
        dQ, dK, dV, dO,
        num_batches,
        seq_len,
        scale
...
/*
빌드:
nvcc -O3 -std=c++17 -arch=sm_86     -o flashattn_streaming_16x16_opt.exe     flashattn_streaming_16x16_opt.cu
