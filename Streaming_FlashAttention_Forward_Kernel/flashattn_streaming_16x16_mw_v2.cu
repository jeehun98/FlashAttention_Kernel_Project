// 7.3 Streaming Softmax FlashAttention-like Multi-Warp (fixed)
    ...
    for (int t = 0; t < num_k_tiles; ++t) {
        ...
        if (lane < WARP_SIZE) {
            for (int i = row_start; i < row_end; ++i) {
                ...
                // --- streaming update (lane 0, row i만 갱신) ---
                if (lane == 0) {
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like Multi-Warp (WMMA QK^T, streaming softmax + PV) [fixed]\n");
