// 7.4 Streaming Softmax FlashAttention-like Multi-Warp + cp.async Double Buffering
// - block당 2 warp (64 threads)
// - warp 0: rows 0..7, warp 1: rows 8..15
// - QK^T: warp 0가 WMMA로 계산 → shared s_scores에 저장
// - V: cp.async + ping/pong double buffering 으로 타일 선로딩
// - streaming softmax + PV는 7.3과 동일한 수식 기반
//
// 빌드:
// nvcc -O3 -std=c++17 -arch=sm_86 \
//   -o flashattn_streaming_16x16_mw_cpasync.exe \
//   flashattn_streaming_16x16_mw_cpasync.cu
//
// 프로파일:
// ncu --set full --launch-skip 10 --launch-count 1 ./flashattn_streaming_16x16_mw_cpasync.exe
...
// === Multi-warp fused kernel: QK^T (WMMA) + streaming softmax + PV + cp.async(V) ===
// Q:  [B, M, Kdim]       half
// K:  [B, Kdim, L]       half
// V:  [B, L, Dv]         half
// O:  [B, M, Dv]         float
__global__ void flashattn_streaming_16x16_kernel_mw_cpasync(
    ...
    // row partition: warp 0 -> 0..7, warp 1 -> 8..15
    ...
    // shared: tile scores, V ping/pong
    __shared__ float          s_scores[M * N_TILE];           // [M, N_TILE]
    __shared__ __align__(16) __half sV_ping[N_TILE * Dv];     // [N_TILE * Dv]
    __shared__ __align__(16) __half sV_pong[N_TILE * Dv];     // [N_TILE * Dv]
    ...
    constexpr int BYTES_PER_TILE = N_TILE * Dv * sizeof(__half); // 16*16*2 = 512
    constexpr int VEC_BYTES      = 16;                           // cp.async 16 bytes
    constexpr int NUM_VECS       = BYTES_PER_TILE / VEC_BYTES;   // 32
#endif

    // === Tile 0 V prefetch (into ping) ===
    if (num_k_tiles > 0) {
#if __CUDA_ARCH__ >= 800
        int col_start0 = 0;
        int tile_offset_h0 = col_start0 * Dv;               // half index
        const __half* src_tile0 = V_b + tile_offset_h0;
        const int4*   src_vec0  = reinterpret_cast<const int4*>(src_tile0);
        int4*         dst_vec0  = reinterpret_cast<int4*>(sV_ping);

        for (int v = tid; v < NUM_VECS; v += blockDim.x) {
            unsigned smem_addr = static_cast<unsigned>(
                __cvta_generic_to_shared(&dst_vec0[v]));
            unsigned long long gmem_addr =
                reinterpret_cast<unsigned long long>(&src_vec0[v]);
            asm volatile(
                "cp.async.ca.shared.global [%0], [%1], %2;\n"
                :: "r"(smem_addr), "l"(gmem_addr), "n"(VEC_BYTES));
        }

        asm volatile("cp.async.commit_group;\n" ::);
        asm volatile("cp.async.wait_group 0;\n" ::);
#else
        // fallback: global->shared direct copy
        int col_start0 = 0;
        for (int idx = tid; idx < N_TILE * Dv; idx += blockDim.x) {
            int j = idx / Dv;
            int d = idx % Dv;
            int kv_index = col_start0 + j;
            sV_ping[idx] = V_b[kv_index * Dv + d];
        }
#endif
        __syncthreads();
    }
    ...
    for (int t = 0; t < num_k_tiles; ++t) {
        ...
        __half* sV_cur  = use_ping ? sV_ping : sV_pong;
        __half* sV_next = use_ping ? sV_pong : sV_ping;
        ...
        // QK 결과 s_scores 사용 전 동기화
        __syncthreads();

        // 2) 다음 타일 V (t+1)를 sV_next에 prefetch (cp.async) — 현재 타일 계산과 overlap
        if (t + 1 < num_k_tiles) {
#if __CUDA_ARCH__ >= 800
            int next_col_start = (t + 1) * N_TILE;
            int tile_offset_h  = next_col_start * Dv;
            const __half* src_tile = V_b + tile_offset_h;
            const int4*   src_vec  = reinterpret_cast<const int4*>(src_tile);
            int4*         dst_vec  = reinterpret_cast<int4*>(sV_next);

            for (int v = tid; v < NUM_VECS; v += blockDim.x) {
                unsigned smem_addr = static_cast<unsigned>(
                    __cvta_generic_to_shared(&dst_vec[v]));
                unsigned long long gmem_addr =
                    reinterpret_cast<unsigned long long>(&src_vec[v]);
                asm volatile(
                    "cp.async.ca.shared.global [%0], [%1], %2;\n"
                    :: "r"(smem_addr), "l"(gmem_addr), "n"(VEC_BYTES));
            }
            ...
            for (int idx = tid; idx < N_TILE * Dv; idx += blockDim.x) {
                ...
                sV_next[idx] = V_b[kv_index * Dv + d];
            }
#endif
        }

        // 3) tile 내 row별 local 통계 + streaming update (각 warp: 자기 row range만)
        if (lane < WARP_SIZE) {
            for (int i = row_start; i < row_end; ++i) {
                ...
                for (int d = 0; d < Dv; ++d) {
                    ...
                    if (lane < N_TILE) {
                        int j = lane;
                        __half hv = sV_cur[j * Dv + d];   // [N_TILE, Dv] row-major
                ...
                // --- streaming update (lane 0만 해당 row 갱신) ---
                if (lane == 0) {
    ...
    // 5) 최종 결과 쓰기: 각 warp가 자기 row range 처리, warp 내 lane-stride store
    for (int i = row_start; i < row_end; ++i) {
...
int main() {
    std::printf("Streaming Softmax FlashAttention-like Multi-Warp + cp.async (WMMA QK^T, streaming softmax + PV)\n");
    ...
    // 1회 실행 (정확도 체크)
    flashattn_streaming_16x16_kernel_mw_cpasync<<<grid, block>>>(
        dQ, dK, dV, dO,
        num_batches,
        seq_len,
        scale
...
/*
빌드:
nvcc -O3 -std=c++17 -arch=sm_86     -o flashattn_streaming_16x16_mw_cpasync.exe     flashattn_streaming_16x16_mw_cpasync.cu
