// flashattn_forward_fused.cu
// 5.4 - Full FlashAttention Forward (FMA + shared memory double-buffer version)
//
...
// --- Full FlashAttention Forward fused kernel (FMA version) ---
// Q, K, V, O: [BH, N, D], row-major
__global__ void flashattn_forward_fused_kernel(
...
{
    if (D > MAX_D) return;  // simple guard for this sample
    ...
    int tid = threadIdx.x;
    int num_threads = blockDim.x;  // 예: 128

    // shared memory layout:
    //  Q_smem: [BLOCK_M, D]   (block lifetime 동안 유지)
    //  K_smem: [2, TILE_N, D] (double-buffer)
    //  V_smem: [2, TILE_N, D] (double-buffer)
    extern __shared__ float smem[];
    float* Q_smem = smem;                                     // BLOCK_M * D
    float* K_smem = Q_smem + BLOCK_M * MAX_D;                 // 2 * TILE_N * D
    float* V_smem = K_smem + 2 * TILE_N * MAX_D;              // 2 * TILE_N * D

    // row-wise softmax state in shared memory
    ...
    // 각 thread는 1개의 query row를 담당 (가능한 범위에서)
    // 전체 block에서 최대 BLOCK_M개의 row를 커버
    ...
    for (int k0 = 0; k0 < N; k0 += TILE_N) {
        ...
        // K_smem[stage][tile_cols, D], V_smem[stage][tile_cols, D]
        ...
        for (int r = row_begin; r < row_end; ++r) {
            ...
            // 2-2-2. incremental softmax update (Stage2)
            // existing row_max / row_sum
            ...
            float exp_scale = (old_l == 0.0f && old_m == -INFINITY)
                              ? 0.0f
                              : expf(old_m - new_m) * old_l;
            ...
            // 2-2-3. weights = exp(score - row_max) / row_sum, then * V (Stage3)
            // O_local[r, :] += Σ_j weight_ij * V_tile[j, :]
            for (int j = 0; j < tile_cols; ++j) {
                float w = exp_scores[j] / new_l;
...
int main() {
    ...
    printf("FlashAttention Forward Fused (5.4, FMA + shared double-buffer)\n");
    printf("BH=%d, N=%d, D=%d\n", BH, N, D);
    ...
    // Warm-up
    flashattn_forward_fused_kernel<<<grid, block, smem_bytes>>>(
        d_Q, d_K, d_V, d_O,
        BH, N, D, scale
    );
    ...
    // FLOP 계산: QK^T (2*N*N*D) + softmax(exp/add) 대충 + P×V(2*N*N*D) ~ 4*N*N*D*BH
...
/*
nvcc -O3 -arch=sm_86 flashattn_forward_fused.cu -o flashattn_forward_fused.exe

ncu --launch-skip 1 --launch-count 1     --kernel-name-base demangled     --kernel-name regex:flashattn_forward_fused_kernel     --section ComputeWorkloadAnalysis     --section MemoryWorkloadAnalysis     --section SpeedOfLight_RooflineChart     .\flashattn_forward_fused.exe

.\flashattn_forward_fused.exe
*/